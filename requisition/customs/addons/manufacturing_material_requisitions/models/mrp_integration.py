...
class MrpProductionExtension(models.Model):
    ...
    requisition_ids = fields.One2many('manufacturing.material.requisition', 'manufacturing_order_id', 'Material Requisitions')
    requisition_count = fields.Integer('Requisition Count', compute='_compute_requisition_count')
    auto_requisition_enabled = fields.Boolean('Auto Requisition Enabled', default=True)
    requisition_analyzed = fields.Boolean('Requisition Analyzed', default=False)
    auto_requisition_id = fields.Many2one('manufacturing.material.requisition', 'Auto-Generated Requisition')
    ...
    def action_create_requisition(self):
        """Create material requisition for production order"""
        return {
            'type': 'ir.actions.act_window',
            'name': 'Create Material Requisition',
            'res_model': 'mrp.requisition.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
    ...
    def action_view_requisitions(self):
        ...
        action = self.env.ref('manufacturing_material_requisitions.action_manufacturing_material_requisition').read()[0]
        action['domain'] = [('manufacturing_order_id', '=', self.id)]
...
class MrpWorkorderExtension(models.Model):
    ...
    material_requisition_ids = fields.Many2many('manufacturing.material.requisition', 
                                                'mrp_workorder_requisition_rel', 
                                                'workorder_id', 'requisition_id',
                                                string='Material Requisitions')
    is_critical_path = fields.Boolean('Critical Path', default=False)
...
class MrpBomExtension(models.Model):
    ...
    auto_requisition_enabled = fields.Boolean('Auto Requisition Enabled', default=True)
    requisition_lead_time = fields.Integer('Requisition Lead Time (Days)', default=7)
...
class ManufacturingMrpIntegration(models.Model):
    _name = 'manufacturing.mrp.integration'
    _description = 'MRP Integration Dashboard'
    
    # Dashboard fields
    auto_requisitions_count = fields.Integer('Auto Requisitions Count', compute='_compute_dashboard_data')
    pending_requisitions = fields.Integer('Pending Requisitions', compute='_compute_dashboard_data')
    material_availability = fields.Float('Material Availability %', compute='_compute_dashboard_data')
    shortage_count = fields.Integer('Shortage Count', compute='_compute_dashboard_data')
    shortage_analysis = fields.Html('Shortage Analysis', compute='_compute_shortage_analysis')
    forecast_data = fields.Text('Forecast Data', compute='_compute_forecast_data')
    
    @api.depends('create_date')
    def _compute_dashboard_data(self):
        for record in self:
            # Auto requisitions count
            auto_reqs = self.env['manufacturing.material.requisition'].search([
                ('auto_generated', '=', True),
                ('create_date', '>=', fields.Date.today() - timedelta(days=30))
            ])
            record.auto_requisitions_count = len(auto_reqs)
            
            # Pending requisitions
            pending = self.env['manufacturing.material.requisition'].search([
                ('state', 'in', ['submitted', 'shop_floor_review', 'supervisor_approval', 'manager_approval'])
            ])
            record.pending_requisitions = len(pending)
            
            # Material availability and shortage count
            productions = self.env['mrp.production'].search([
                ('state', 'in', ['confirmed', 'progress']),
                ('date_planned_start', '<=', fields.Date.today() + timedelta(days=7))
            ])
            ...
            record.material_availability = (available_materials / total_materials * 100) if total_materials else 100
            record.shortage_count = shortages
    
    @api.depends('create_date')
    def _compute_shortage_analysis(self):
        for record in self:
            html = """
            <div class="o_mrp_shortage_analysis">
                <h4>Current Material Shortages</h4>
            ...
            for production in productions:
                for move in production.move_raw_ids:
                    if move.reserved_availability < move.product_uom_qty:
                        shortage_qty = move.product_uom_qty - move.reserved_availability
                        html += f"""
                        <tr>
                            <td>{move.product_id.name}</td>
                            <td>{move.product_uom_qty:.2f}</td>
                            <td>{move.reserved_availability:.2f}</td>
                            <td class="text-danger">{shortage_qty:.2f}</td>
            ...
            record.shortage_analysis = html
    
    @api.depends('create_date')
    def _compute_forecast_data(self):
        for record in self:
            # Generate forecast data for chart
            record.forecast_data = json.dumps({
                'labels': ['Week 1', 'Week 2', 'Week 3', 'Week 4'],
                'datasets': [{
                    'label': 'Forecasted Requisitions',
                    'data': [15, 22, 18, 25],
    ...
    def action_analyze_shortages(self):
        """Analyze material shortages and create requisitions"""
        # Run MRP analysis
        integration = self.env['mrp.requisition.integration']
        integration.run_mrp_requisition_analysis()
        
        # Refresh dashboard
        self._compute_dashboard_data()
        self._compute_shortage_analysis()
        
        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': _('MRP Analysis Complete'),
                'message': _('Material shortage analysis completed. Check the dashboard for results.'),
                'type': 'success',
                'sticky': False,
            }
        } 