...
class InventoryIntegration(models.Model):
    ...
    # Related Records
    requisition_ids = fields.One2many('manufacturing.material.requisition', 'inventory_integration_id', 'Generated Requisitions')
    ...
    def _create_auto_requisition(self):
        ...
        for record in self:
            ...
            existing_requisition = self.env['manufacturing.material.requisition'].search([
                ('state', 'in', ['draft', 'submitted', 'approved', 'in_progress']),
                ('line_ids.product_id', '=', record.product_id.id),
            ...
            if not existing_requisition:
                ...
                requisition_vals = {
                    'department_id': self.env.user.department_id.id,
                    'priority': '2',  # High priority for reorders
                    'state': 'draft',
                    'requisition_date': fields.Date.today(),
                    'required_date': fields.Date.today() + timedelta(days=7),
                    'notes': 'Auto-generated reorder requisition',
                    'line_ids': [(0, 0, {
                        'product_id': record.product_id.id,
                        'quantity': quantity,
                        'uom_id': record.product_id.uom_id.id,
                        'required_date': fields.Date.today() + timedelta(days=7),
                        'notes': f'Auto-reorder: Stock reached {record.current_stock}'
                    })]
                }
...
class StockMoveExtension(models.Model):
    ...
    requisition_id = fields.Many2one('manufacturing.material.requisition', 'Material Requisition')
    requisition_line_id = fields.Many2one('manufacturing.material.requisition.line', 'Requisition Line')
