...
class RequisitionAnalytics(models.Model):
    ...
    # Basic Information
    requisition_id = fields.Many2one('manufacturing.material.requisition', 'Requisition', readonly=True)
...
class RequisitionDashboard(models.Model):
    ...
    def _get_alerts(self):
        ...
        # Check overdue requisitions
        today = fields.Date.today()
        overdue_requisitions = self.env['manufacturing.material.requisition'].search([
            ('required_date', '<', today),
        ...
        if overdue_requisitions:
            alerts.append({
                'type': 'overdue',
                'severity': 'high',
                'message': f'{len(overdue_requisitions)} overdue requisitions need attention',
                'action': 'Review and expedite overdue requisitions'
            })
        
        # Check emergency requisitions
        emergency_requisitions = self.env['manufacturing.material.requisition'].search([
...
class ShopFloorAnalytics(models.Model):
    ...
    # Add missing fields for analytics dashboard
    avg_completion_time = fields.Float('Average Completion Time', compute='_compute_analytics')
    sla_percentage = fields.Float('SLA Percentage', compute='_compute_analytics')
    emergency_count = fields.Integer('Emergency Count', compute='_compute_analytics')
    total_cost = fields.Monetary('Total Cost', compute='_compute_analytics')
    avg_cost_per_requisition = fields.Monetary('Avg Cost per Requisition', compute='_compute_analytics')
    cost_variance = fields.Float('Cost Variance %', compute='_compute_analytics')
    currency_id = fields.Many2one('res.currency', default=lambda self: self.env.company.currency_id)
    
    # Chart data fields
    requisition_chart = fields.Text('Requisition Chart Data', compute='_compute_chart_data')
    department_chart = fields.Text('Department Chart Data', compute='_compute_chart_data')
    material_analysis = fields.Text('Material Analysis Data', compute='_compute_chart_data')
    
    @api.depends('requisition_id')
    def _compute_analytics(self):
        for record in self:
            # Get all analytics records for calculations
            all_analytics = self.search([])
            
            if all_analytics:
                record.avg_completion_time = sum(all_analytics.mapped('completion_time')) / len(all_analytics)
                sla_met_count = len(all_analytics.filtered('sla_met'))
                record.sla_percentage = (sla_met_count / len(all_analytics)) * 100
                
                # Get related requisitions
                requisitions = all_analytics.mapped('requisition_id')
                record.emergency_count = len(requisitions.filtered('is_emergency'))
                record.total_cost = sum(requisitions.mapped('total_amount'))
                record.avg_cost_per_requisition = record.total_cost / len(requisitions) if requisitions else 0
                
                # Calculate cost variance
                if record.avg_cost_per_requisition > 0:
                    variance = sum(abs(r.total_amount - record.avg_cost_per_requisition) for r in requisitions)
                    record.cost_variance = (variance / len(requisitions) / record.avg_cost_per_requisition) * 100
                else:
                    record.cost_variance = 0
            else:
                record.avg_completion_time = 0
                record.sla_percentage = 0
                record.emergency_count = 0
                record.total_cost = 0
                record.avg_cost_per_requisition = 0
                record.cost_variance = 0
    ...
    def _compute_chart_data(self):
        for record in self:
            ...
            record.requisition_chart = json.dumps({
            ...
            record.department_chart = json.dumps({
                'labels': ['Production', 'Maintenance', 'Quality', 'Warehouse'],
                'data': [300, 50, 100, 80]
            })
            
            record.material_analysis = json.dumps({
                'rows': [
                    ['Product A', 100, 2500],
                    ['Product B', 50, 1200],
                    ['Product C', 75, 1800]
