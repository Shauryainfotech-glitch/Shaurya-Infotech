...
class AccountFiscalYear(models.Model):
    ...
    def _check_dates(self):
        ...
        for fy in self:
            ...
            # Search for overlapping fiscal years within the same company
            domain = [
                ('company_id', '=', fy.company_id.id),
                ('id', '!=', fy.id),  # Exclude the current fiscal year
                '|', '|',
                ('date_from', '<=', fy.date_to), ('date_to', '>=', fy.date_from),
                ('date_from', '<', fy.date_from), ('date_to', '>', fy.date_from),
                ('date_from', '<', fy.date_to), ('date_to', '>', fy.date_to),
            ]
            if self.search_count(domain) > 0:
                raise ValidationError(_('You cannot have an overlap between two fiscal years, '
