...
class ArchitectController(http.Controller):
    ...
    # Dashboard Statistics API endpoints for models
    @http.route('/architect/api/dashboard/projects', type='json', auth='user')
    def get_project_dashboard_stats(self, **kwargs):
    ...
    @http.route('/architect/api/dashboard/compliance', type='json', auth='user')
    def get_compliance_dashboard_stats(self, **kwargs):
    ...
    @http.route('/architect/api/dashboard/dpr', type='json', auth='user')
    def get_dpr_dashboard_stats(self, **kwargs):
        """Get DPR dashboard statistics"""
        try:
            dpr_items = request.env['architect.dpr'].search([])
            return {
                'success': True,
                'data': {
                    'total': len(dpr_items),
                    'draft': len(dpr_items.filtered(lambda d: d.state == 'draft')),
                    'under_review': len(dpr_items.filtered(lambda d: d.state == 'under_review')),
                    'approved': len(dpr_items.filtered(lambda d: d.state == 'approved'))
                }
            }
        except Exception as e:
            return {'success': False, 'error': str(e)}