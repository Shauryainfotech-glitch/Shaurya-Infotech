...
class ArchitectRateSchedule(models.Model):
    ...
    def action_duplicate_for_new_period(self):
        """Create a new version for the next period"""
    ...
    def action_apply_escalation(self):
        """Apply price escalation to all items"""
...
class ArchitectRateComponent(models.Model):
    ...
    currency_id = fields.Related('rate_item_id.currency_id', store=True)
...
class ArchitectEstimation(models.Model):
    ...
    overhead_amount = fields.Monetary(string='Overhead Amount', compute='_compute_amounts', store=True, currency_field='currency_id')
    profit_percentage = fields.Float(string='Profit (%)', default=10.0)
    profit_amount = fields.Monetary(string='Profit Amount', compute='_compute_amounts', store=True, currency_field='currency_id')
    total_before_tax = fields.Monetary(string='Total Before Tax', compute='_compute_amounts', store=True, currency_field='currency_id')
    tax_amount = fields.Monetary(string='Tax Amount', compute='_compute_amounts', store=True, currency_field='currency_id')
    total_amount = fields.Monetary(string='Total Amount', compute='_compute_amounts', store=True, currency_field='currency_id')
    ...
    def _compute_amounts(self):
        for estimation in self:
            ...
            estimation.tax_amount = estimation.total_before_tax * 0.18  # Assuming 18% GST
...
class ArchitectEstimationLine(models.Model):
    ...
    unit_rate = fields.Monetary(string='Unit Rate', related='rate_item_id.unit_rate', readonly=True, currency_field='currency_id')
    total_amount = fields.Monetary(string='Total Amount', compute='_compute_total_amount', store=True, currency_field='currency_id')
    
    currency_id = fields.Related('estimation_id.currency_id', store=True)
