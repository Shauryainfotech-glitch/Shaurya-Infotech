...
class MrpEstimationCost(models.Model):
    ...
    # Cost fields
    unit_cost = fields.Monetary(string='Unit Cost', currency_field='currency_id', default=0.0)
    ...
    def _compute_total_cost(self):
        for cost in self:
            if cost.cost_type == 'operation':
                cost.total_cost = cost.operation_time * (cost.hourly_rate or 0.0)
            elif cost.cost_type == 'labor':
                base_cost = cost.labor_hours * (cost.labor_rate or 0.0)
                overhead_cost = base_cost * (cost.labor_overhead / 100)
                cost.total_cost = base_cost + overhead_cost
            else:
                cost.total_cost = (cost.unit_cost or 0.0) * cost.quantity
