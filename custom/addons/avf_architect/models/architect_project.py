...
class ArchitectProject(models.Model):
    ...
    # Project timeline
    start_date = fields.Date(string='Project Start Date', default=fields.Date.today)
    ...
    architect_id = fields.Many2one('res.users', string='Lead Architect')
    ...
    def _check_dates(self):
        for record in self:
            if record.expected_end_date and record.start_date > record.expected_end_date:
                raise ValidationError(_('Start date cannot be later than expected end date.'))
